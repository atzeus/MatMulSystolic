// This changes ZelfSystolicNice by making sure each channel is written/read 
...
channel vec_float_t		 ch_data_b_border       
...
{ 
    // this horrible code is curtesy of the only one mention of channel
    // limitation
    bool sync = true;
    bool first = true;
    bool flushMove = false;
    bool flush = false;
    bool done = false;
    int rowBlock = 0;
    int reuse = 0;
    ...
    int row = 0;
    while(!done){
        bool load = !sync && !flush && !flushMove;
	...
	unsigned int dotProdVecLength)
{
	for(int reuse = 0 ; reuse < nrYBlocks; reuse++){
		for(int colBlock = 0 ; colBlock < nrXBlocks ; colBlock++){
            bool flush = reuse == nrYBlocks -1 && colBlock == nrXBlocks - 1;
            int nrRows;
            if(flush){
                nrRows = dotProdVecLength+2;
            } else {
                nrRows = dotProdVecLength;
            }
...
__kernel void buf_mat_b_kernel()
{
	...
	while(true){
		for(int reuse = 0 ; reuse < INTERLEAVED ; reuse++){
		     for(int i = 0 ; i < INTERLEAVED ; i++){
                write_channel_intel(ch_data_b_border[col],buf[bufIndex][i]);
            }
